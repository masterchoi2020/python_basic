...
# yscrollcommand=scrollbar.set 을 해 줘야 연동이 가능함(스크롤바를 리스트 박스와 맵핑)
list_file = Listbox(list_frame, selectmod="extended", height=15, yscrollcommand=scrollbar.set)

list_file.pack(side="left", fill="both", expand=True)  # fill="both" : 위아래, 좌우로 펼침
scrollbar.config(command=list_file.yview)  # yview로 scrollbar 매핑시키기(리스트박스와 스크롤봐 맵핑)
...
# fill="x" : 좌우로 펼침(fill="y" : 상하로 펼침)
# ipady=4 : inner padding -> 높이 변경(박스 안쪽을 넓이는 효과)
