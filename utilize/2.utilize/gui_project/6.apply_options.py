...
from tkinter import *    # __all__ 에 정의하지 않으면 서브모듈은 별도 명시해 줘야 한다.
from tkinter import filedialog  # 서브모듈 별도 명시적으로 import해 줌
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
            ...
            initialdir = r"D:\python_workspace")  
...
def merge_image():
    ...
    try:
        ...
        if img_width == "원본유지":
            img_width = -1  # -1 일때는 원본 기준으로 통합 필요함으로 별도 처리
        else:
        ...
        ### 이미지 크기 변환 계산식 ####
        # 100 x 60 이미지가 있음 ->  width를 80으로 줄이면 height 는 ?
        ...
        y_offset = 0  # y 위치 정보: 이미지를 위에서 하나씩 붙여나갈때 y 좌표

        for idx, img in enumerate(images):
            # width 가 "원본유지"가 아닐 때에는 이미지 크지 조정
            if img_width > -1:
        ...
        # 포멧 옵션 처리
        file_name = "KITT_photo." + img_format
