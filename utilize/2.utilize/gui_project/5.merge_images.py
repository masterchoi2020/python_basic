...
from tkinter import *   # __all__에 정의하지 않으면 서브모듈은 별도 명시해 줘야 한다.
from tkinter import filedialog   # 서브모듈 별도 명시적으로 import 해 줌
...
def merge_image():
    ...
    # size의 실제 저장방식 : [ (10, 10), (20, 20), (30, 30)]

    # zip을 이용해 위의 widths, heights를 좀더 간편하게 코드 작성
    ...
    y_offset = 0 # y 위치 정보: 이미지를 위에서 하나씩 붙여나갈 때 y 좌표
    
    # for img in images:
    #     result_img.paste(img, (0, y_offset))  # 이미지 붙여 넣기 x=0, y=y_offset 기준으로 붙여 넣어짐
    #     y_offset += img.size[1]   # 이미지 붙인 후 높이 값 변경 (height 값 만큼 더해 준다.)
    ...
    dest_path = os.path.join(txt_dest_path.get(), "KITT_photo.jpg")
...
txt_dest_path.pack(fill="x", padx=5, pady=5, ipady=5)
