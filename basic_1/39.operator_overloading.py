# 연산자 오버로딩
# 부모클래스에서 정의한 내용이 아닌 자식 클래스에서 정의한 내용을 다시 사용하고 싶을 때 사용
...
# 벌처 유닛 생성 : 지상 유닛, 기동성이 좋음
vulture = AttackUint("벌쳐", 80, 10, 20)  #체력 80, 스피드 10, 공격력 20

# 배틀 크루저: 공중 유닉, 체력 /공격력 좋음
battlecruiser = FlyableAttackUnit("배틀크루저", 500, 25, 3)  #체력 500, 스피드 25, 공격력 3

# 문제점 : 지상 유닛은 move() 함수 사용, 공중유닛은 fly()함수 사용 => 지상유닛인지 공중유닛인지 매번 확인 필요
# 연산자 오버로딩을 통해 move 함수하나로 통일 해서 사용가능
vulture.move("11시")
